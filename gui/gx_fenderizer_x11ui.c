...
static void resize_event(gx_fenderizerUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_fenderizerUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_fenderizerUI* ui = (gx_fenderizerUI*)malloc(sizeof(gx_fenderizerUI));
...
static void cleanup(LV2UI_Handle handle) {
	gx_fenderizerUI* ui = (gx_fenderizerUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(gx_fenderizerUI *ui,gx_controller* knob) {
	...
	cairo_pattern_add_color_stop_rgba (pat, 0.75,  0.4, 0.4, 0.4, 1.0);
	cairo_pattern_add_color_stop_rgba (pat, 0.5,  0.6, 0.6, 0.6, 1.0);
	cairo_pattern_add_color_stop_rgba (pat, 0.25,  0.4, 0.4, 0.4, 1.0);
	cairo_pattern_add_color_stop_rgba (pat, 1,  0.2, 0.2, 0.2, 1.0);
	...
	cairo_fill_preserve (ui->crf);
 	cairo_set_source_rgb (ui->crf, 0.3, 0.3, 0.3); 
	...
	cairo_set_source (ui->crf, pat);
	cairo_fill_preserve (ui->crf);
 	cairo_set_source_rgb (ui->crf, 0.35, 0.35, 0.35); 
	cairo_set_line_width(ui->crf,1);
	cairo_stroke(ui->crf);
	cairo_new_path (ui->crf);
	...
	pat = cairo_pattern_create_linear (0, 0, 0, knob_y);
	cairo_pattern_add_color_stop_rgba (pat, 1,  0.2, 0.2, 0.2, 1.0);
	cairo_pattern_add_color_stop_rgba (pat, 0.5,  0.4, 0.4, 0.4, 1.0);
	cairo_pattern_add_color_stop_rgba (pat, 0.,  0.6, 0.6, 0.6, 1.0);
	...
	cairo_set_source_rgb (ui->crf,0.73,0.73,0.73);
	cairo_stroke(ui->crf);
	cairo_new_path (ui->crf);
...
static void bypass_expose(gx_fenderizerUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void controller_expose(gx_fenderizerUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
...
static void send_controller_event(gx_fenderizerUI *ui, int controller) {
...
static void check_value_changed(gx_fenderizerUI *ui, int i, float* value) {
...
static void check_is_active(gx_fenderizerUI *ui, int i, bool set) {
...
static bool aligned(int x, int y, gx_controller *control, gx_fenderizerUI *ui) {
...
static bool get_active_ctl_num(gx_fenderizerUI *ui, int *num) {
	static bool ret;
	ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num(gx_fenderizerUI *ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_fenderizerUI *ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_fenderizerUI *ui, int i) {
	float value;
...
static void motion_event(gx_fenderizerUI *ui, double start_value, int m_y) {
...
static void set_key_value(gx_fenderizerUI *ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_fenderizerUI *ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_fenderizerUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_fenderizerUI *ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_fenderizerUI *ui, bool set) {
...
static void event_handler(gx_fenderizerUI *ui) {
	XEvent xev;
